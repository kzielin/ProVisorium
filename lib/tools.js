$(function() {
   $("div.tabs").tabs();
   $("tr.appear input,tr.appear select,tr.appear textarea,tr.appear button")
   ...
   $("textarea.autoheight")
      ...
      .attr('data-maxrows', function(){
         return parseInt($(this).attr('data-maxrows')) > 0 ? $(this).attr('data-maxrows') : 30;
      })
      .keyup(function(){
         while (this.scrollHeight > this.clientHeight && $(this).css('overflow-y') == 'hidden') {
            if (this.rows < $(this).attr('data-maxrows')) this.rows = this.rows + 1;
   ...
   ;
   $('input[type="file"]')
      .wrap('<div style="margin:0;padding:0;display:block;width:100px;height:21px;overflow:hidden;" class="ui-corner-all button" />')
      .css({opacity: 0,"margin-top":-1,"margin-left":-1})
      .before('<div style="position:absolute;margin: 2px 0 0 5px;"><span class="ui-icon ui-icon-inline ui-icon-folder-open" />Wybierz plik</div>')
      ;
...
function screenHolderClick(e, ob) {
   ...
   if ($('.iconHolderSelected:first').length == 1) {
      ...
      var newOb = $('<div><span class="pv-movable-handle ui-icon ui-icon-arrow-4"></span>'
                     +window.atob(compo.attr('data-html'))+'</div>');
      $(ob).append(newOb);
