...
class Projekt
{
    ...
    static function lista($login = null) /**/
    {
        ...
        if (is_null($login))
            ...
            $r = $db->query("SELECT * FROM projects WHERE id in (SELECT project FROM users2projects WHERE login = '$login' )  ORDER BY name");;
        return Db::fetch_all($r);
    }

    static function dodaj($nazwa, $theme)
    {
        $db = Db::getInstance();
        $nazwa = Db::escape($nazwa);
        $theme = (int)$theme;
        $db->exec("INSERT INTO projects (name,theme) VALUES ('$nazwa', $theme);");
        return $db->changes();
    }
    ...
    public static function getDefaultScreenId($projectId)
    {
        $db = Db::getInstance();
        if (!is_numeric($projectId)) return false;
        return $db->querySingle("SELECT id FROM screens WHERE projectId = '$projectId' and isMain");
    }
